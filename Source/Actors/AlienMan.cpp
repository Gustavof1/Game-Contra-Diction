    ...
    , mHasPlayedActiveSound(false)
{
    mAnimatorComponent = new AnimatorComponent(this, 
        "../Assets/Sprites/AlienMan/casual_alien_guy_idle_default.png", 
        "../Assets/Sprites/AlienMan/casual_alien_guy_idle_default.json", 
        90, 128, 110);
    
    mAnimatorComponent->AppendSpriteSheet(
        "../Assets/Sprites/AlienMan/casual_alien_guy_running_default.png",
        "../Assets/Sprites/AlienMan/casual_alien_guy_running_default.json");
    
    mAnimatorComponent->AppendSpriteSheet(
        "../Assets/Sprites/AlienMan/casual_alien_guy_walking_walking_pace.png",
        "../Assets/Sprites/AlienMan/casual_alien_guy_walking_walking_pace.json");

    mAnimatorComponent->AppendSpriteSheet(
        "../Assets/Sprites/AlienMan/casual_alien_guy_death_default.png",
        "../Assets/Sprites/AlienMan/casual_alien_guy_death_default.json");
    ...
    mAnimatorComponent->SetSize(45.0f, 64.0f);
...
void AlienMan::OnUpdate(float deltaTime)
{
    if (mIsDying) {
    ...
    if (playerShooting) {
        ...
        if (dist < mMaintainDistance) {
            ...
            mIsRunningAway = false;
            if (playerPos.x < mPosition.x) {
                velocity.x = mMoveSpeed; // Move right (away from player on left)
            } else {
                velocity.x = -mMoveSpeed; // Move left (away from player on right)
            }
        } else {
...
void AlienMan::ManageAnimations()
{
    if (mIsDying) {
...
void AlienMan::OnHorizontalCollision(const float minOverlap, AABBColliderComponent* other)
{
